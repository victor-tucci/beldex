...
std::optional<uint64_t> expiry_blocks(cryptonote::network_type nettype, mapping_type type)
{
  ...
  if (is_belnet_type(type))
  {
    ...
    if (type == mapping_type::belnet)              result = BLOCKS_EXPECTED_IN_DAYS(1 * REGISTRATION_YEAR_DAYS);
    else if (type == mapping_type::belnet_2years)  result = BLOCKS_EXPECTED_IN_DAYS(2 * REGISTRATION_YEAR_DAYS);
    else if (type == mapping_type::belnet_5years)  result = BLOCKS_EXPECTED_IN_DAYS(5 * REGISTRATION_YEAR_DAYS);
    else if (type == mapping_type::belnet_10years) result = BLOCKS_EXPECTED_IN_DAYS(10 * REGISTRATION_YEAR_DAYS);
    assert(result);
