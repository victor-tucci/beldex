...
bool TimingsDatabase::load()
{
  ...
  if (!f)
  {
    oxen::log::debug("Failed to load timings file {}: {}", filename, strerror(errno));
    return false;
  }
  while (1)
  {
    ...
    if (!tab)
    {
      oxen::log::warn("Bad format: no tab found");
      continue;
    }
    ...
    {
      oxen::log::error("Bad format: wrong number of fields: got {} expected {}", fields_sv.size(), N_EXPECTED_FIELDS);
      continue;
    }
...
bool TimingsDatabase::save()
{
  ...
  if (!f)
  {
    oxen::log::error("Failed to write to file {}: {}", filename, strerror(errno));
    return false;
  }
