...
if (m{^\s*<script>\s*$} .. m{^\s*</script>\s*$}) {
  ...
  <script>
    window.\$docsify = {
      name: "Beldex Daemon RPC",
      repo: "https://github.com/Beldex-Coin/beldex",
    ...
    s{<title>.*</title>}{<title>Beldex Daemon RPC</title>};
    s{(name="description" content=)"[^"]*"}{$1"Beldex Daemon RPC endpoint documentation"};
    if (m{^\s*</body>}) {
        print qq{
