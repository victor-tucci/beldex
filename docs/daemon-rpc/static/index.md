# Beldex Daemon RPC Endpoints

These pages describe the available RPC endpoints available from a running `beldexd` node.  These
endpoints are used for querying blockchain data, submitting transactions, obtaining master node
information, and controlling the running beldexd.
...
call the `get_info` endpoint on a master node with an HTTP RPC listener on `localhost:22023` (the
...
All beldexd endpoints are also available via OxenMQ at either the `rpc.ENDPOINT` or `admin.ENDPOINT`
...
The beldex-core source code contains a script (`utils/lmq-rpc.py`) that can invoke such a request:

```
./utils/lmq-rpc.py ipc://$HOME/.beldex/beldexd.sock rpc.get_info | jq .
```
to query a local beldexd, or:
```
...
beldexd's X25519 pubkey; the second version uses an unencrypted public connection).